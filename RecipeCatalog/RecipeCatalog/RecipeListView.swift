        ...
        .overlay {
            if recipes.isEmpty {
                ContentUnavailableView {
                    Label("No recipes in this category", systemImage: "x.circle")
                } description: {
...
#Preview("RecipeListView") {
    ModelContainerPreview(ModelContainer.sample) {
        NavigationStack {
            RecipeListView(recipeCategoryName: RecipeCategory.snack.name)
...
#Preview("No recipes") {
    ModelContainerPreview(ModelContainer.sample) {
        RecipeList(recipeCategoryName: RecipeCategory.breakfast.name)
            .environment(NavigationContext())
    }
}
